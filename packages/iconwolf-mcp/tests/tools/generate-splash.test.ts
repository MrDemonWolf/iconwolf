...
import { handleGenerateSplash } from '../../src/tools/generate-splash.js';
...
describe('handleGenerateSplash', () => {
  it('generates 1024x1024 splash icon from file_path', async () => {
    const tmpDir = createTmpDir();
    dirsToClean.push(tmpDir);
    const inputPath = await createTestPng(1024, 1024, tmpDir);
    const outputDir = tmpDir + '/output';

    const result = await handleGenerateSplash({
      file_path: inputPath,
