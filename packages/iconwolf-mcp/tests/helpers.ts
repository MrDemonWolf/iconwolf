...
export async function createTestPng(
  ...
  const tmpDir =
    dir ?? fs.mkdtempSync(path.join(os.tmpdir(), 'iconwolf-mcp-test-'));
...
export async function createTestPngBase64(
  width: number,
  height: number,
): Promise<string> {
...
export function createTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'iconwolf-mcp-test-'));
}
